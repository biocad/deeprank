...
class DataSetForPretrainedModelRAM(DataSet):
    def __init__(self, train_database, valid_database=None, test_database=None,
                 chain1='A', chain2='B',
                 chain_mappings=None,
                 mapfly=True, grid_info=None,
    ...
    def process_dataset(self):
        ...
        logger.info('=')
        logger.info('=\t Training data')
        for f in self.train_database.mol_list:
            logger.info(f'=\t -> {f}')
        logger.info('=')
        if self.valid_database:
            logger.info('=\t Validation data')
    ...
    def create_index_molecules(self):
        ...
        for fdata in data_tqdm:
            ...
            try:
                # fh5 = h5py.File(fdata, 'r')
                mol_names = list(fdata.mol_list)
                # mol_names = self._select_pdb(mol_names)
                for k in mol_names:
                    if self.filter(fdata.data_dict[k]):
                        self.index_complexes += [(fdata,
                                                  k, None, None)]
                        for irot in range(self.data_augmentation):
                            axis, angle = pdb2sql.transform.get_rot_axis_angle(
    ...
    def get_mapped_feature_name(self):
        ...
        # open a h5 file in case we need it
        data_dict = self.train_database.data_dict
        mol_names = self.train_database.mol_list
        mol_name = mol_names[0]
        mapped_data = data_dict[mol_name]['mapped_features']
        chain_tags = ['_chain1', '_chain2']
    ...
    def get_grid_shape(self):
        ...
        if self.mapfly is False:
            ...
            fname = self.train_database
            # fh5 = h5py.File(fname, 'r')
            mol = fname.mol_list[0]

            # get the mol
            mol_data = fname.data_dict[mol]
    ...
    def load_one_molecule(self, fname, mol=None):
        ...
        if mol is None:
            mol = self.train_database.mol_list[0]
            # mol = list(fh5.keys())[0]

        # get the mol
        mol_data = self.train_database.data_dict[mol]

        # xue:
        if 'mapped_features' not in mol_data.keys():
            logger.error(f"xue: Error: mol: {mol} does not have mapped_features ")
            # fh5.close()
            sys.exit()
        ...
        for feat_type, feat_names in self.select_feature.items():
            ...
            # see if the feature exists
            if feat_type in mol_data['mapped_features'].keys():
            # if 'mapped_features/' + feat_type in mol_data.keys():
                feat_dict = mol_data['mapped_features'][feat_type]
            else:
                logger.error(
                    f'Feature type {feat_type} not found'
                    f'for molecule {mol}.\n'
                    f'Possible feature types are:\n\t' +
                    '\n\t'.join(
                        list(mol_data['mapped_features'].keys()))
            ...
            for name in feat_names:
                ...
                try:
                    ...
                    logger.error(
                        f'Feature {name} not found for mol '
                        f'{mol} and feature type {feat_type}.\n'
                        f'Possible feature are:\n\t' +
                        '\n\t'.join(list(
        ...
        # get the target value
        target = mol_data['targets'][self.select_target]
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            return {'mol': [mol], 'feature': feature, 'target': target}
